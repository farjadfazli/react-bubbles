...
A token is used to authenticate yourself to the server.
...
Web servers receive requests from clients and respond by serving the requested data (provided that the request is properly authenticated with a token if required). They can also receive data from clients and store it to a database for later retrieval.
